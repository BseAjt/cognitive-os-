...
export interface DecisionFrame {
  question: string;
  category: "hiring" | "investment" | "launch" | "pricing" | "partnership" | "workforce_restructuring" | "generic";
  criteria: string[];
...
export function buildDecisionFrame(message: string, challenge: Challenge): DecisionFrame {
  ...
  if (WORKFORCE_RESTRUCTURING_PATTERN.test(lower)) {
    return {
      ...
      options: [
        {
          title: "Préserver l’emploi par des mesures alternatives",
          description: "Examiner réduction des dépenses, gel des recrutements, mobilité, non-remplacement, activité partielle ou autres leviers adaptés.",
          score: null
        },
        {
          title: "Mettre en œuvre une restructuration intermédiaire",
          description: "Étudier les départs volontaires, réorganisations, reclassements et mesures d’accompagnement avant toute suppression contrainte.",
          score: null
        },
        {
          title: "Envisager des suppressions de postes",
          description: "N’instruire cette option que si la nécessité est établie, les alternatives insuffisantes et les obligations sociales et juridiques cadrées.",
          score: null
        }
      ],
      recommendation: null,
