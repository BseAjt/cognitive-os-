...
const PROJECTS = [
  ...
  ["project-platform", "Trustworthy AI Platform", "Industrialiser une plateforme IA explicable, observable et souveraine.", "Produit", "CTO", "Perspective de faisabilité", "active", "now", 88, 95, 86, 5, 64, ["demo-platform"], []],
  ["project-growth", "Revenue Engine 2027", "Construire le dispositif commercial reproductible pour l'Enterprise.", "Go-to-market", "CRO", "Perspective stratégique", "validated", "next", 86, 89, 74, 6, 27, ["demo-hiring"], ["project-nova"]],
  ["project-copilot", "Executive Copilot", "Transformer les décisions de direction en exécution observable.", "Innovation", "CEO", "Assistant de décision", "validated", "next", 91, 98, 71, 7, 18, [], ["project-platform"]],
  ...
  ["project-academy", "Executive AI Academy", "Accélérer l'adoption par une académie de décision augmentée.", "Adoption", "COO", "Perspective de prudence", "discovery", "next", 68, 76, 63, 4, 12, [], ["project-copilot"]],
  ["project-regulated", "Sovereign Vertical", "Adapter la plateforme aux secteurs défense et services publics.", "Verticalisation", "CEO", "Perspective stratégique", "on_hold", "later", 90, 87, 54, 9, 6, [], ["project-platform"]],
  ["project-intelligence", "Market Intelligence Network", "Détecter en continu les signaux qui fragilisent la stratégie.", "Intelligence", "CSO", "Assistant de décision", "active", "now", 84, 93, 79, 5, 39, [], ["project-nova"]]
] as const;
...
const IDEAS = [
  ...
  ["idea-simulator", "Decision Simulator", "Les conséquences des options restent implicites.", "Simuler scénarios, hypothèses et seuils avant arbitrage.", "Produit", "Perspective de faisabilité", "evaluating", "next", 91, 90, 68, 66, "project-copilot", undefined, ["simulation", "scenario"]],
  ...
  ["idea-benchmark", "Decision Benchmark", "Les dirigeants ne savent pas calibrer leur processus de décision.", "Comparer anonymement vitesse, traçabilité et révisions par secteur.", "Data", "Perspective de prudence", "captured", "later", 88, 83, 52, 48, undefined, undefined, ["benchmark", "privacy"]],
  ...
  ["idea-digital-twin", "Cognitive Twin Calibration", "Le système doit apprendre sans figer les biais de l'utilisateur.", "Mesurer préférences, calibration et évolution du raisonnement avec consentement.", "Cognitive AI", "Assistant de décision", "evaluating", "later", 96, 94, 49, 55, "project-platform", undefined, ["twin", "ethics"]],
  ["idea-red-team", "Autonomous Red Team", "Les décisions consensuelles masquent des risques.", "Déclencher automatiquement une contradiction argumentée avant les gates critiques.", "Risque", "Perspective de prudence", "promoted", "now", 85, 87, 89, 83, "project-intelligence", "demo-partner", ["risk", "challenge"]],
  ...
  ["idea-sovereign", "Sovereign Deployment Kit", "Les secteurs régulés ne peuvent pas adopter une architecture standard.", "Packager déploiement souverain, audit, isolation et politiques sectorielles.", "Verticalisation", "Perspective de faisabilité", "captured", "later", 74, 93, 57, 51, "project-regulated", undefined, ["sovereignty", "enterprise"]],
  ["idea-academy", "Decision Practice Lab", "L'adoption nécessite de nouvelles habitudes managériales.", "Faire pratiquer des cas de décision avec feedback de calibration.", "Adoption", "Perspective de prudence", "promoted", "next", 69, 72, 88, 77, "project-academy", undefined, ["learning", "adoption"]],
  ["idea-radar", "Weak Signal Radar", "Les décisions sont réévaluées trop tard.", "Surveiller sources internes et externes selon les hypothèses actives.", "Intelligence", "Perspective stratégique", "promoted", "now", 82, 91, 84, 81, "project-intelligence", "demo-pricing", ["signals", "watch"]],
  ["idea-outcomes", "Outcome Ledger", "La valeur réelle des décisions est rarement mesurée.", "Relier chaque décision à ses résultats, coûts et apprentissages dans le temps.", "Finance", "Assistant de décision", "captured", "next", 79, 89, 73, 64, undefined, undefined, ["outcomes", "roi"]]
] as const;
...
export function createInvestorDemoDataset(): InvestorDemoDataset {
  ...
  const actions: ActionRecord[] = cases.flatMap((cognitiveCase, caseIndex) => Array.from({ length: 6 }, (_, actionIndex) => ({
    ...
    owner: ["Perspective stratégique", "CFO", "Perspective de prudence", "Perspective de faisabilité", "Assistant de décision", "CEO"][actionIndex]!, progress: actionIndex < 2 ? 100 : actionIndex === 2 ? 75 : actionIndex === 3 ? 45 : 15,
  ...
  const executiveCycles: ExecutiveCycleRecord[] = cases.map((cognitiveCase, index) => ({
    ...
    contributions: [
      { agentId: "athena", agentName: "Perspective stratégique", mandate: "Stratégie", position: "support", analysis: "La trajectoire progressive maximise l'apprentissage tout en préservant les options.", confidence: 87, evidenceIds: [`demo-evidence-${index + 1}-1-1`], citations: ["S1"] },
      { agentId: "turing", agentName: "Perspective de faisabilité", mandate: "Exécution", position: "conditional", analysis: "Le passage à l'échelle dépend des deux intégrations critiques et d'un KPI de délai.", confidence: 84, evidenceIds: [`demo-evidence-${index + 1}-5-1`], citations: ["S5"] },
      { agentId: "seneca", agentName: "Perspective de prudence", mandate: "Risques", position: "challenge", analysis: "Le signal concurrentiel et la contrainte réglementaire imposent un gate de réévaluation.", confidence: 91, evidenceIds: [`demo-evidence-${index + 1}-6-1`], citations: ["S6"] }
    ],
    divergences: index < 4 ? [{ topic: "Vitesse contre maîtrise du risque", agentIds: ["athena", "seneca"], description: "Perspective stratégique privilégie la vitesse ; Perspective de prudence demande un gate de contrôle.", resolution: "Pilote limité et checkpoint à six semaines." }] : [],
    synthesis: "Assistant de décision recommande une décision progressive, sourcée et réversible.", recommendation: "Activer le scénario progressif avec indicateurs et checkpoint.", confidence: 86, missingEvidence: [], sourceIds: contextSources.filter((source) => source.caseId === cognitiveCase.id).map((source) => source.id), createdAt: at(14 + index)
  }));

  const decisionActionPlans: DecisionActionPlanRecord[] = cases.map((cognitiveCase, index) => ({ id: `demo-plan-${index + 1}`, caseId: cognitiveCase.id, executiveCycleId: `demo-cycle-${index + 1}`, decisionId: `demo-decision-${index + 1}-1`, recommendation: executiveCycles[index]!.recommendation!, status: "active", actionIds: actions.filter((action) => action.caseId === cognitiveCase.id).map((action) => action.id), dependencies: [], metrics: [{ id: `demo-metric-${index + 1}`, label: "Progression du plan", target: "100 %", current: `${Math.round(actions.filter((action) => action.caseId === cognitiveCase.id).reduce((sum, action) => sum + action.progress, 0) / 6)} %`, owner: "Assistant de décision" }], checkpointAt: at(28 + index), createdAt: at(16 + index) }));

  const decisionWatches: DecisionWatchRecord[] = decisionActionPlans.map((plan, index) => ({ id: `demo-watch-${index + 1}`, caseId: plan.caseId, planId: plan.id, decisionId: plan.decisionId, status: index < 3 ? "reopen" : index === 3 ? "watch" : "stable", signals: index < 3 ? [{ id: `demo-signal-${index + 1}`, type: "contradiction", severity: "critical", title: "Hypothèse fragilisée", detail: "Un signal postérieur à la décision contredit l'hypothèse initiale.", sourceId: `demo-source-${index + 1}-6`, evidenceId: `demo-evidence-${index + 1}-6-1`, citation: "S6" }] : [], summary: index < 3 ? "La décision doit être rouverte." : "La décision reste sous contrôle.", recommendedAction: index < 3 ? "Relancer un cycle Assistant de décision ciblé." : "Poursuivre le suivi.", evaluatedAt: at(26 + index) }));
...
export const INVESTOR_DEMO_STEPS = [
  ...
  { id: "reasoning", label: "2. Raisonnement", detail: "Perspective stratégique, Perspective de faisabilité et Perspective de prudence confrontent leurs positions." },
  { id: "decision", label: "3. Décision", detail: "Assistant de décision produit une recommandation sourcée et réversible." },
