...
export function runConversationRuntime(message: string, challenge: Challenge): RuntimeResult {
  ...
  const extractions = sentences.map((sentence) => classify(sentence));
  ...
  const challengePatch: Partial<Challenge> = {
    ...
    urgency: decision || action ? Math.min(10, challenge.urgency + 1) : challenge.urgency,
    state: decision ? "decide" : action ? "execute" : challenge.state
  };

  const nextAction = action?.text || buildNextAction(intent, challenge, risk, hypothesis);
  const response = buildResponse(intent, challenge, extractions, nextAction);

  return { intent, extractions, response, nextAction, challengePatch };
}
...
function detectIntent(lower: string): RuntimeResult["intent"] {
  ...
  if (/je dois décider|je veux décider|dois-je|quelle décision/.test(lower)) return "decision";
...
function classify(sentence: string): CognitiveExtraction {
