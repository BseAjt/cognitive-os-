...
export function buildDecisionToAction(input:DecisionToActionInput):DecisionToActionResult {
  if(input.cycle.caseId!==input.cognitiveCase.id) throw new Error("Le cycle ORION n’appartient pas à ce dossier.");
  if(input.cycle.status!=="completed"||!input.cycle.recommendation) throw new Error("Une recommandation ORION validée est requise avant l’exécution.");
  ...
  const actions:ActionRecord[]=[
    ...
    {id:actionIds[2]!,caseId:input.cognitiveCase.id,title:"Mesurer le résultat et tenir le checkpoint ORION",owner:"ORION",progress:0,status:"todo",requiredCapability:"decision",assignedAgentId:"orion",dueAt:addDays(createdAt,14)}
  ];
  const plan:DecisionActionPlanRecord={id:input.id??crypto.randomUUID(),caseId:input.cognitiveCase.id,executiveCycleId:input.cycle.id,decisionId,recommendation:input.cycle.recommendation,status:"active",actionIds,dependencies:[{actionId:actionIds[1]!,dependsOnActionId:actionIds[0]!},{actionId:actionIds[2]!,dependsOnActionId:actionIds[1]!}],metrics:[{id:`${decisionId}:progress`,label:"Actions terminées",target:"3/3",current:"0/3",owner:"ORION"},{id:`${decisionId}:confidence`,label:"Confiance de décision",target:`≥ ${input.cycle.confidence}%`,current:`${input.cycle.confidence}%`,owner:owners.strategy}],checkpointAt:addDays(createdAt,14),createdAt};
  const decision:DecisionRecord={id:decisionId,caseId:input.cognitiveCase.id,recommendation:input.cycle.recommendation,outcome:input.cycle.objective,rationale:`Décision issue du cycle ORION ${input.cycle.id}, fondée sur ${input.cycle.sourceIds.length} source(s) et ${input.cycle.contributions.length} perspective(s).`,confidence:input.cycle.confidence,createdAt};
...
function ownersFor(cycle:ExecutiveCycleRecord):{strategy:string;delivery:string} { const byId=new Map(cycle.contributions.map((item)=>[item.agentId,item.agentName])); return {strategy:byId.get("athena")??"ATHENA",delivery:byId.get("turing")??"TURING"}; }
