...
export function CloudSyncStatus(){
  ...
  const organizationId=useExecutiveStore((state)=>state.activeOrganizationId);
  ...
  useEffect(()=>{
    if(!userEmail || !organizationId || revision!==0)return;
    void sync(organizationId);
    // Sync once when the authenticated organization becomes available.
    // eslint-disable-next-line react-hooks/exhaustive-deps
  },[userEmail,organizationId,revision]);
  async function sync(organizationOverride?:string){
    setStatus("syncing");
    const targetOrganizationId=organizationOverride??useExecutiveStore.getState().activeOrganizationId;
    if(!targetOrganizationId){setStatus("error");return;}
    const remote=await fetch(`/api/cloud/workspace?organizationId=${encodeURIComponent(targetOrganizationId)}`,{cache:"no-store"});
    ...
    if(!local){setStatus("error");return;}
    const saved=await fetch("/api/cloud/workspace",{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({organizationId:targetOrganizationId,revision,payload:local})});
  ...
  if(status==="checking" || status==="local")return null;
  ...
  return <aside aria-live="polite" className="fixed right-3 top-[4.5rem] z-40 flex max-w-[calc(100vw-1.5rem)] items-center gap-2 rounded-full border border-black/10 bg-[#fbfaf7]/95 p-1.5 pl-3 text-xs text-[#3a3a3c] shadow-md shadow-black/5 backdrop-blur-xl sm:right-4 lg:top-4"><span className={`size-2 shrink-0 rounded-full ${status==="synced"?"bg-emerald-600":status==="conflict"||status==="error"?"bg-amber-600":"bg-[#007aff]"}`}/><span className="hidden sm:inline" title={userEmail}>{labels[status]}</span><button onClick={()=>void sync()} disabled={status==="syncing"} className="min-h-10 rounded-full bg-[#007aff] px-3 py-2 font-semibold text-white disabled:opacity-50">Sync</button><button onClick={signOut} aria-label="Se déconnecter" className="min-h-10 rounded-full px-2 py-2 text-[#6e6e73]">Quitter</button></aside>;
}
