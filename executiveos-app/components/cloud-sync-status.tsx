...
export function CloudSyncStatus(){
  ...
  return <aside aria-live="polite" className="fixed right-4 top-4 z-50 flex items-center gap-2 rounded-full border border-black/10 bg-[#fbfaf7]/90 p-1.5 pl-3 text-xs text-[#3a3a3c] shadow-lg shadow-black/5 backdrop-blur-xl"><span className={`size-2 rounded-full ${status==="synced"?"bg-emerald-500":status==="conflict"||status==="error"?"bg-amber-500":status==="local"?"bg-zinc-400":"bg-[#007aff]"}`}/><span title={userEmail}>{labels[status]}</span>{status==="local"?<a href="/sign-in" className="rounded-full bg-black px-3 py-1.5 font-semibold text-white">Connecter</a>:<><button onClick={sync} disabled={status==="syncing"} className="rounded-full bg-[#007aff] px-3 py-1.5 font-semibold text-white disabled:opacity-50">Sync</button><button onClick={signOut} aria-label="Se déconnecter" className="rounded-full px-2 py-1.5 text-[#6e6e73]">Quitter</button></>}</aside>;
}
