...
export function ExecutiveWorkspace() {
  ...
  function sendMessage() {
    ...
    const result = runConversationRuntime(message, active);
    const updated = { ...active, ...result.challengePatch };
    const now = new Date().toISOString();

    store.updateChallenge(updated);
    store.addMessages([
      { id: crypto.randomUUID(), challengeId: active.id, role: "user", text: message, createdAt: now },
    ...
    if (decision) {
      store.addDecision({
        ...
        rationale: "Décision extraite de la conversation et conservée dans le Decision Ledger.",
    ...
    if (action) {
      store.addActions([{ id: crypto.randomUUID(), challengeId: active.id, title: action.text, owner: "À assigner", progress: 0, status: "todo" }]);
      store.addEvent("ActionCreated", action.text);
    }
    ...
    <div className="grid min-h-screen grid-cols-[270px_1fr] max-md:grid-cols-1">
      <aside className="border-r border-white/10 bg-[#050b15]/90 p-5 max-md:hidden">
        ...
        <div className="mt-3 grid gap-2">
          {ranked.map((challenge) => (
            <button
              key={challenge.id}
              onClick={() => {
                store.setActiveChallenge(challenge.id);
                setLastExtractions([]);
                setLastNextAction("");
              }}
              className={`rounded-xl border p-3 text-left ${active.id === challenge.id ? "border-[#7c5cff]/50 bg-[#7c5cff]/10" : "border-white/10 bg-white/[.02]"}`}
            >
        ...
        <div className="mt-8 rounded-2xl border border-white/10 bg-white/[.025] p-4">
          ...
          <div className="mt-3 grid gap-2 text-sm text-[#cfd8e8]">
            <span>● Context Engine</span><span>● Scheduler</span><span>● Decision Ledger</span><span>● Cognitive Graph</span>
          </div>
        </div>
      </aside>
      ...
      <main className="min-w-0">
        <header className="sticky top-0 z-10 flex h-[72px] items-center justify-between border-b border-white/10 bg-[#07111f]/80 px-7 backdrop-blur">
          <div>ExecutiveOS / <strong>{active.title}</strong></div>
          <div className="flex gap-2">
            <button onClick={() => setShowGraph(!showGraph)} className="executive-button executive-ghost">{showGraph ? "Masquer" : "Afficher"} le graphe</button>
            <button onClick={store.runCriticalSimulation} className="executive-button executive-ghost">Signal critique</button>
          </div>
        </header>
        ...
        <section className="mx-auto max-w-[1550px] p-6">
          <div className="mb-5">
            ...
            <p className="mt-2 text-[#91a2bd]">Parle naturellement. ExecutiveOS structure le raisonnement et met à jour les moteurs en arrière-plan.</p>
          </div>
            ...
            <article className="executive-card overflow-hidden">
              <div className="max-h-[560px] min-h-[500px] overflow-auto p-5">
                {messages.length ? messages.map((message) => (
                  <div key={message.id} className={`mb-4 flex ${message.role === "user" ? "justify-end" : "justify-start"}`}>
                    <div className={`max-w-[82%] rounded-2xl px-4 py-3 leading-7 ${message.role === "user" ? "bg-[#6b49df] text-white" : "border border-white/10 bg-[#16243c] text-[#e8edf6]"}`}>
                      {message.text}
                    </div>
                  </div>
              ...
              <div className="border-t border-white/10 p-4">
                <div className="flex gap-3 max-sm:flex-col">
                  ...
                  />
                  <button onClick={sendMessage} className="executive-button executive-primary self-end px-6 py-4">Analyser</button>
                </div>
                <div className="mt-3 flex flex-wrap gap-2">
                  {["J’ai une idée", "Je dois prendre une décision", "J’ai un problème", "Reprendre là où j’en étais"].map((prompt) => (
                    <button key={prompt} onClick={() => setInput(prompt + " : ")} className="rounded-full border border-white/10 px-3 py-2 text-xs text-[#91a2bd]">{prompt}</button>
                  ))}
                </div>
              </div>
            </article>

            {showGraph && <div className="grid gap-5">
              <article className="executive-card p-5">
                ...
                <div className="mt-4 grid grid-cols-3 gap-3">
                  <Metric label="Priority" value={challengeScore(active)} />
                ...
                <div className="mt-4 rounded-2xl border border-white/10 bg-white/[.025] p-4">
                  ...
                  <p className="mt-2 text-[#d6dfed]">{lastNextAction || active.context}</p>
                </div>
              </article>

              <ReasoningGraph challenge={active} />

              <article className="executive-card p-5">
                <div className="flex items-center justify-between">
                  <div className="text-xs font-black tracking-[.14em] text-[#8d7ce4]">STRUCTURED OUTPUT</div>
                  <span className="text-xs text-[#91a2bd]">{lastExtractions.length} objets</span>
                </div>
                <div className="mt-3 grid gap-2">
                  {lastExtractions.length ? lastExtractions.map((item, index) => (
          ...
          <div className="mt-5 grid grid-cols-2 gap-5 max-lg:grid-cols-1">
            <article className="executive-card p-5">
              ...
              <div className="mt-3 grid gap-3">
                {decisions.length ? decisions.slice(0, 3).map((decision) => <div key={decision.id} className="rounded-xl border border-white/10 p-3"><strong>{decision.finalDecision}</strong><p className="mt-1 text-sm text-[#91a2bd]">{decision.rationale}</p></div>) : <span className="text-sm text-[#91a2bd]">Aucune décision détectée.</span>}
              </div>
            </article>
            <article className="executive-card p-5">
              <div className="text-xs font-black tracking-[.14em] text-[#ffbc57]">OPEN ACTIONS</div>
              <div className="mt-3 grid gap-3">
                {actions.length ? actions.slice(0, 4).map((action) => <div key={action.id} className="rounded-xl border border-white/10 p-3"><strong>{action.title}</strong><p className="mt-1 text-sm text-[#91a2bd]">{action.owner} · {action.status}</p></div>) : <span className="text-sm text-[#91a2bd]">Aucune action détectée.</span>}
              </div>
            </article>
          </div>
        </section>
