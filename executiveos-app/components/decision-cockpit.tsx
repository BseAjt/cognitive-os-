...
import type { ContextReadiness } from "@/lib/context-engine";
...
export function DecisionCockpit({ frame, readiness, portfolio, onRecord, onCreateAction }: Props) {
  const council = useMemo(() => conveneExecutiveCouncil([], readiness, portfolio), [readiness, portfolio]);
  ...
  return <article className="executive-card mt-5 overflow-hidden">
    <div className="border-b border-white/10 bg-[#12172a] p-5">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div><div className="text-xs font-black tracking-[.14em] text-[#ffbc57]">DECISION COCKPIT</div><h2 className="mt-2 text-2xl font-semibold">Arbitrer, décider et rendre la décision révisable</h2><p className="mt-2 text-sm text-[#91a2bd]">Vue consolidée du contexte, des scénarios, des objections, des conditions et des déclencheurs de révision.</p></div>
        <span className={`rounded-full px-3 py-1 text-xs ${cockpit.decisionAllowed ? "bg-[#42d59d]/15 text-[#7aefc2]" : "bg-[#ff7185]/15 text-[#ff9dab]"}`}>{cockpit.decisionAllowed ? "Prêt à décider" : "Décision bloquée"}</span>
      </div>
    </div>
    ...
    <div className="grid gap-5 p-5 xl:grid-cols-[.8fr_1.2fr]">
      ...
      <section className="grid content-start gap-4">
        <div className="grid gap-3 md:grid-cols-2"><label className="grid gap-2 text-sm">Décideur<input value={owner} onChange={(e) => setOwner(e.target.value)} className="rounded-xl border border-white/10 bg-[#0d1727] p-3" /></label><label className="grid gap-2 text-sm">Échéance<input type="date" value={deadline} onChange={(e) => setDeadline(e.target.value)} className="rounded-xl border border-white/10 bg-[#0d1727] p-3" /></label></div>
        <label className="grid gap-2 text-sm">Rationale du dirigeant<textarea value={rationale} onChange={(e) => { setRationale(e.target.value); setSaved(false); }} className="min-h-28 rounded-xl border border-white/10 bg-[#0d1727] p-3" /></label>
        <div className="grid gap-4 lg:grid-cols-2"><div className="rounded-2xl border border-white/10 p-4"><strong>Conditions</strong><div className="mt-3 grid gap-2 text-sm text-[#aebbd0]">{cockpit.conditions.map((item) => <span key={item}>• {item}</span>)}</div></div><div className="rounded-2xl border border-white/10 p-4"><strong>Opinions dissidentes</strong><div className="mt-3 grid gap-2 text-sm text-[#aebbd0]">{cockpit.dissentingViews.length ? cockpit.dissentingViews.map((item) => <span key={item}>• {item}</span>) : <span>Aucune objection bloquante.</span>}</div></div></div>
