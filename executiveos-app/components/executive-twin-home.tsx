...
function TopNav({ view, onView }: { view: View; onView: (view: View) => void }) {
  const items: Array<[View, string]> = [["home", "Executive Twin"], ["decision", "Decision Room"], ["memory", "Organizational Memory"], ["graph", "Enterprise Graph"]];
  return <header className="sticky top-0 z-30 border-b border-white/10 bg-[#07111f]/90 px-6 py-4 backdrop-blur-xl"><div className="mx-auto flex max-w-[1500px] flex-wrap items-center justify-between gap-4"><button onClick={() => onView("home")} className="flex items-center gap-3"><span className="grid size-10 place-items-center rounded-2xl bg-gradient-to-br from-[#9d83ff] to-[#5c39e8] font-black">ET</span><span className="text-left"><strong className="block">Executive Twin</strong><span className="text-xs text-[#91a2bd]">Release 1 Foundation</span></span></button><nav className="flex flex-wrap gap-2">{items.map(([id, label]) => <button key={id} onClick={() => onView(id)} className={`rounded-xl px-4 py-2 text-sm ${view === id ? "bg-white/10 text-white" : "text-[#91a2bd] hover:bg-white/5"}`}>{label}</button>)}</nav></div></header>;
}
  ...
  return <>
    <section className="grid gap-6 xl:grid-cols-[1.2fr_.8fr]">
      <article className="executive-card overflow-hidden p-7 md:p-9">
        <div className="text-xs font-black tracking-[.18em] text-[#9d83ff]">EXECUTIVE BRIEFING</div>
        ...
        <p className="mt-6 max-w-3xl text-lg leading-8 text-[#b8c4d6]">Ton Twin consolide le contexte, la mémoire organisationnelle et les décisions ouvertes. Il ne recommande aucune action irréversible tant que les preuves critiques sont incomplètes.</p>
