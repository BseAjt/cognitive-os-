...
export function OrionExecutiveCyclePanel({caseId}:{caseId:string}) {
  ...
  function run(){
    try { setError(""); store.prependExecutiveCycle(runOrionExecutiveCycle({objective,cognitiveCase:active!,agents:store.agents,sources:store.contextSources,evidence:store.contextEvidence})); }
  ...
  return <div className="space-y-5">
    <article className="executive-card p-5">
      ...
      <div className="mt-4 flex gap-3 max-sm:flex-col"><input value={objective} onChange={(event)=>setObjective(event.target.value)} placeholder="Quel arbitrage ORION doit-il préparer ?" className="min-w-0 flex-1 rounded-xl border border-white/10 bg-[#091422] px-4 py-3 text-sm outline-none"/><button onClick={run} className="rounded-xl bg-[#42b98d] px-5 py-3 text-sm font-bold text-[#071711]">Lancer le cycle</button></div>{error&&<p role="alert" className="mt-3 text-xs text-red-300">{error}</p>}
