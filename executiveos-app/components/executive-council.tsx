    ...
    <article className="executive-card mt-5 overflow-hidden">
      <div className="border-b border-white/10 bg-[#101b2f] p-5">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            <div className="text-xs font-black tracking-[.14em] text-[#42d59d]">EXECUTIVE COUNCIL · Assistant de décision</div>
            <h2 className="mt-2 text-2xl font-semibold">Débat exécutif structuré</h2>
            <p className="mt-2 max-w-3xl text-sm leading-6 text-[#91a2bd]">Chaque agent évalue le même dossier selon son mandat. Assistant de décision expose les objections, les désaccords et les conditions qui doivent être satisfaites avant décision.</p>
          </div>
      ...
      <div className="grid gap-5 p-5 xl:grid-cols-[.75fr_1.25fr]">
        <section className="grid content-start gap-3">
          {council.assessments.map((assessment) => <button key={publicCopy(assessment.agent)} onClick={() => setActiveAgent(assessment.agent)} className={`rounded-2xl border p-4 text-left ${activeAgent === assessment.agent ? "border-[#42d59d]/60 bg-[#42d59d]/10" : "border-white/10 bg-white/[.02]"}`}>
            <div className="flex items-center justify-between gap-3"><div><strong>{publicCopy(assessment.agent)}</strong><span className="ml-2 text-xs text-[#91a2bd]">{assessment.role}</span></div><span className="text-xs text-[#91a2bd]">{assessment.confidence}%</span></div>
