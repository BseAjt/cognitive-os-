...
export const createExecutiveCommands: StateCreator<ExecutiveState, [], [], ExecutiveCommands> = (set, get) => ({
  applyRuntimeCycle: ({ caseId, userText, result, createdAt }) => {
    ...
    set((state) => {
      ...
      const previousRecall = currentCase ? buildCognitiveRecall({
        cognitiveCase: currentCase,
      ...
      const cognitiveDiff = currentCase ? buildCognitiveDiff({
        ...
        previousKnowledge: state.knowledgeRecords,
      ...
      const decision = result.decision ? {
        ...
        rationale: result.decision.rationale, confidence: result.decision.confidence, createdAt: timestamp
      } : undefined;

      const actions = result.actions.map((action) => ({
        id: crypto.randomUUID(), caseId, title: action.title, owner: action.preferredAgentName ?? "À assigner", progress: 0,
        status: "todo" as const, requiredCapability: action.requiredCapability, assignedAgentId: action.preferredAgentId ?? null
      }));

      const agentRun = {
        id: crypto.randomUUID(), caseId, orchestratorId: result.agents.orchestratorId, selectedAgentIds: result.agents.selectedAgentIds,
        contributions: result.agents.contributions, synthesis: result.agents.synthesis, confidence: result.agents.confidence, createdAt: timestamp
      };

      const memories = result.memory.map((memory) => ({
        id: crypto.randomUUID(), caseId, kind: memory.kind, content: memory.content, confidence: memory.confidence,
        durable: memory.durable, source: "unified_runtime" as const, createdAt: timestamp
      }));

      const knowledgeRecords = result.knowledge.map((knowledge) => ({
        id: crypto.randomUUID(), caseId, type: knowledge.type, title: knowledge.title, confidence: knowledge.confidence,
        source: "unified_runtime" as const, createdAt: timestamp
      }));

      const reasoningRevisions = result.reasoning.map((revision) => {
        const existingCount = state.reasoningRevisions.filter((item) => item.caseId === caseId && item.stepId === revision.stepId).length;
        return { id: crypto.randomUUID(), caseId, stepId: revision.stepId, version: existingCount, content: revision.content,
          confidence: revision.confidence, risk: revision.risk, createdAt: timestamp };
      });
  ...
  recordConversationTurn: ({ caseId, userText, assistantText, intent, extractionCount, casePatch, createdAt }) => {
    ...
    set((state) => ({
      ...
      events: [{ id: crypto.randomUUID(), type: "ConversationParsed", detail: `${intent} · ${extractionCount} objets détectés`, createdAt: timestamp }, ...state.events]
    }));
  },
  ...
  captureDecision: ({ caseId, text, recommendation, confidence, rationale, createdAt }) => {
    const timestamp = createdAt ?? new Date().toISOString();
    set((state) => ({
      decisions: [{ id: crypto.randomUUID(), caseId, recommendation, outcome: text, rationale: rationale ?? "Décision extraite de la conversation.", confidence, createdAt: timestamp }, ...state.decisions],
  ...
  createAction: ({ caseId, title, owner, requiredCapability }) => {
    ...
    set((state) => ({
      actions: [{ id: crypto.randomUUID(), caseId, title, owner: owner ?? "À assigner", progress: 0, status: "todo", requiredCapability }, ...state.actions],
