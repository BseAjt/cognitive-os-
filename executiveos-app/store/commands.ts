...
export const createExecutiveCommands: StateCreator<ExecutiveState, [], [], ExecutiveCommands> = (set, get) => ({
  ...
  applyCriticalSignal: () => { const timestamp=new Date().toISOString(); const activeCaseId=get().activeCaseId; set((state)=>({cases:state.cases.map((cognitiveCase)=>cognitiveCase.id===activeCaseId?{...cognitiveCase,context:"Les utilisateurs pilotes remettent en cause la disposition à payer sans intégration calendrier.",signals:{...cognitiveCase.signals,confidence:41,urgency:10,risk:9}}:cognitiveCase),events:[{id:crypto.randomUUID(),type:"CriticalSignalDetected",detail:"La disposition à payer devient incertaine.",createdAt:timestamp},...state.events]})); }
});
