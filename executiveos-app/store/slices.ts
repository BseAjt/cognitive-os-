...
import type { ExecutiveState, CaseSlice, ConversationSlice, DecisionSlice, ActionSlice, EventSlice, LearningSlice, ReflectionSlice, CognitiveProfileSlice, MemorySlice, KnowledgeSlice, KnowledgeGraphSlice } from "./types.ts";
import {
  initialCases,
  initialMessages,
...
export const createCaseSlice: StateCreator<ExecutiveState, [], [], CaseSlice> = (set) => ({ cases: initialCases, activeCaseId: "executiveos", setActiveCase: (id) => set({ activeCaseId: id }), replaceCase: (cognitiveCase) => set((state) => ({ cases: state.cases.map((item) => item.id === cognitiveCase.id ? cognitiveCase : item) })), applyCasePatch: (caseId, patch) => set((state) => ({ cases: state.cases.map((item) => item.id === caseId ? { ...item, ...patch } : item) })) });
