...
const contracts: Array<[string, string, string]> = [
  ...
  ["Executive Kernel owns runtime pipeline execution", executiveKernel, "result = runtime(input)"],
  ["Executive Kernel recovery remains bounded", executiveKernel, "while (true)"],
  ["Executive Kernel delegates failure classification to recovery planner", executiveKernel, "const plan = recovery(error, attemptCount)"],
